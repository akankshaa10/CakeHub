...
switch($_GET["action"]) {
  case "add":
    if(!empty($_POST["quantity"])) {
      $productByCode = $db_handle->runQuery("SELECT * FROM premiumtbl WHERE code='" . $_GET["code"] . "'");
...
<HEAD>
<TITLE>Premium Cake</TITLE>
<link href="premiumstyle.css" type="text/css" rel="stylesheet" />
</HEAD>
<BODY>
<div id="shopping-cart">
<div class="txt-heading" style="color: white">Shopping Cart</div>

<a id="btnEmpty" href="premium.php?action=empty">Empty Cart</a>
<?php
...
<?php   
    ...
    ?>
        ...
        <td style="text-align:center;"><a href="premium.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction"><img src="icon-delete.png" alt="Remove Item" /></a></td>
        </tr>
        <?php
...
<div id="product-grid">
  ...
  <?php
  $product_array = $db_handle->runQuery("SELECT * FROM premiumtbl ORDER BY id ASC");
  if (!empty($product_array)) { 
    foreach($product_array as $key=>$value){
  ?>
    <div class="product-item">
      <form method="post" action="premium.php?action=add&code=<?php echo $product_array[$key]["code"]; ?>">
